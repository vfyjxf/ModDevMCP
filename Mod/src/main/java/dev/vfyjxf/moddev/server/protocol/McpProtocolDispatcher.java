...
public final class McpProtocolDispatcher {
    ...
    public Map<String, Object> initializedNotification() {
        ...
        var payload = statusTool
                ...
                .map(dev.vfyjxf.moddev.server.api.ToolResult::value)
    ...
    private Map<String, Object> toolDescriptor(dev.vfyjxf.moddev.server.api.McpToolDefinition definition) {
        return Map.of(
    ...
    private Map<String, Object> handleToolCall(Object id, Map<String, Object> params) {
        ...
        if (toolName.startsWith("moddev.")) {
            return handleLocalToolCall(id, dev.vfyjxf.moddev.server.api.ToolResult.failure(selection.error()));
        }
        return errorResponse(id, METHOD_NOT_FOUND, "Method not found", Map.of("tool", toolName));
    }

    private Map<String, Object> handleLocalToolCall(Object id, dev.vfyjxf.moddev.server.api.ToolResult result) {
        if (result.success()) {
            var structuredContent = result.value();
            return successResponse(id, Map.of(
                    "content", List.of(Map.of(
                            "type", "text",
                            "text", new dev.vfyjxf.moddev.server.transport.JsonCodec().writeString(structuredContent)
                    )),
