...
public final class FallbackRegionUiDriver implements UiDriver {
    ...
    @Override
    public dev.vfyjxf.mcp.api.ui.UiInteractionState interactionState(UiContext context) {
        var sessionState = sessionStates.stateFor(context, DESCRIPTOR.id());
        if (!sessionState.open()) {
            return new dev.vfyjxf.mcp.api.ui.UiInteractionState(null, null, null, null, context.mouseX(), context.mouseY(), false, "closed", DESCRIPTOR.id());
        }
        var snapshot = snapshot(context, SnapshotOptions.DEFAULT);
        return new dev.vfyjxf.mcp.api.ui.UiInteractionState(
