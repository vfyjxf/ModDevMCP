...
public final class AnnotationMcpRegistrarLookup<T> {

    private static final Logger LOGGER = LoggerFactory.getLogger(AnnotationMcpRegistrarLookup.class);

    private final Class<T> registrarClass;
    private final Supplier<? extends Collection<String>> classNameSupplier;

    public AnnotationMcpRegistrarLookup(Class<T> registrarClass, Class<? extends Annotation> annotationClass) {
        this(registrarClass, classNamesFor(annotationClass));
    }

    AnnotationMcpRegistrarLookup(Class<T> registrarClass, Supplier<? extends Collection<String>> classNameSupplier) {
    ...
    public Collection<T> findRegistrars() {
        ...
        for (String className : classNameSupplier.get()) {
            try {
                ...
                LOGGER.error("MCP registrar class not found: {}", className, exception);
            } catch (NoSuchMethodException exception) {
                LOGGER.error("MCP registrar {} must expose a public no-arg constructor", className, exception);
            } catch (InvocationTargetException | InstantiationException | IllegalAccessException exception) {
                LOGGER.error("Failed to instantiate MCP registrar: {}", className, exception);
            }
        }
        return registrars;
    }
    ...
    private static Supplier<Collection<String>> classNamesFor(Class<? extends Annotation> annotationClass) {
        ...
        return () -> {
            ...
            try {
                ...
                LOGGER.debug("NeoForge ModList is unavailable; MCP registrar scan is skipped", exception);
                return classNames;
            }
            if (modList == null) {
                return classNames;
